...
DO BEGIN
	...
	DELETE datastore.tblmaster as master from datastore.tblmaster as master inner join system.tblglobal_individual as individual on individual.email_hash = master.email_hash where individual.tsAdded < current_date() and individual.tsAdded > date_sub(current_date(), interval 1 day) ;
...
DO BEGIN
	...
	DELETE datastore.tblmaster as master from datastore.tblmaster as master inner join datastore.tblbounces as bounces on bounces.email_hash = master.email_hash where bounces.date_added < current_date() and bounces.date_added > date_sub(current_date(), interval 1 day) ;
...
DO BEGIN
	...
	SELECT @records := count(*) from system.tblglobal_domains ;
        ...
        SELECT group_concat(domain_name separator ";") from system.tblglobal_domains where tsAdded < current_date() and tsAdded > date_sub(current_date(), interval 1 day) into @domain_wildcards ;
...
DO BEGIN
	...
	SELECT @records := count(*) from system.tblglobal_role ;
        ...
        SELECT group_concat(role_name separator ";") from system.tblglobal_role where tsAdded < current_date() and tsAdded > date_sub(current_date(), interval 1 day) into @roles ;
