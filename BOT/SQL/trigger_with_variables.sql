...
-- 		send the data to  system.services_billing_item
		insert into system.services_billing_item (AccountID, ID_SERVICE, DateAdded, Price, OtherNotes, ChargedByAccountID, BillingAccountID, ExtraDetailTitle, CreatedBy,DateAddedDort) values(OLD.AccountID, OLD.ID_SERVICE, now(), OLD.price, CONCAT("online_purchase_log id = ",OLD.id," list_code = ",OLD.list_code," num_of_records = ",OLD.num_of_records),OLD.user_id,dBillingAccountID,CONCAT("list_code = ",OLD.list_code),OLD.user_id, date_format(now(),'%Y-%m-%d')) ;
	end if ;
end ;
