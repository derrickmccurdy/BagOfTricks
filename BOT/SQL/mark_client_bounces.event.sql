...
--	datastore.tblbounces, emarketing.settings, emarketing.campaigns, system.ls_aliases
		declare cur1 cursor for select date_sub(now(), interval 1 hour ) as sdate, concat("d",campaigns.AccountID, ".", campaigns.ListID) as TableID, campaigns.AccountID as AccountID
			...
			where aliases.ip = this_ip and bounces.date_added >= date_sub(now(), interval 15 minute ) group by campaigns.ListID order by campaigns.AccountID ;
...
--					SET @update_string := CONCAT("update ",@TableID," as client_table inner join datastore.tblbounces as bounces on client_table.email_hash = bounces.email_hash  set client_table.Status = bounces.Status where bounces.date_added >= \"", @dsdate, "\" and bounces.custid = ",@AccountID ) ;
					SET @update_string := CONCAT("update ",@TableID," as client_table inner join datastore.tblbounces as bounces on client_table.email_hash = bounces.email_hash and bounces.custid = ",@AccountID," and bounces.date_added >= \"",@dsdate,"\" set client_table.Status = bounces.Status " ) ;
